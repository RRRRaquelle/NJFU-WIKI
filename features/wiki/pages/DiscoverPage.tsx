...
function readAdvisorSession() {
  ...
  try {
    const stored = JSON.parse(sessionStorage.getItem(ADVISOR_SESSION_KEY) ?? "{}") as {
      turns?: ChatTurn[];
...
export default function DiscoverPage({
  state, query = "", resourceItems, onNavigate, onSearch, onOpenDrawer, onSave, onLoginRequired, isLoggedIn,
  ...
  const searchResults = resourceItems.filter((r) => {
    const matchQuery = query
      ? r.title.includes(query) || r.tags.some((t) => t.includes(query)) || r.type.includes(query)
      : true;
    const matchFilter = filter === "全部" || r.type === filter;
    return matchQuery && matchFilter;
  });

  const askAdvisor = async (text: string, requestProfile: StudentProfile, replaceConversation = false) => {
    const userTurn: ChatTurn = { role: "user", content: text };
    setChatTurns((prev) => replaceConversation ? [userTurn] : [...prev, userTurn]);
    setGenerating(true);
    ...
    try {
      ...
      const payload = await response.json() as { reply?: AssistantReply; error?: string };
      ...
      const message = error instanceof Error ? error.message : "暂时无法连接导学助手";
      setChatTurns((prev) => [
        ...prev,
        {
          role: "ai",
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem(ADVISOR_SESSION_KEY, JSON.stringify({ turns: chatTurns, profile }));
    } catch {
      ...
      <div className="min-h-screen bg-[var(--color-paper)] flex flex-col">
        ...
        <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-6 py-8 gap-4">
          ...
          <div className="flex items-center justify-between mb-2">
            <Button variant="text" size="sm" onClick={() => onNavigate("discover", "default")}>
              ← 返回发现
            </Button>
      ...
      <div className="min-h-screen bg-[var(--color-paper)] flex flex-col">
        ...
        <main className="flex-1 max-w-6xl mx-auto w-full px-[clamp(24px,4vw,64px)] py-10 flex flex-col gap-8">
          ...
          <div className="flex flex-wrap gap-2">
            {FILTERS.map((f) => (
              <Chip key={f} label={f} variant="filter" active={filter === f} onClick={() => setFilter(f)} />
            ))}
          </div>
          ...
          {searchResults.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
              ...
              <p className="text-lg font-serif font-bold text-[var(--color-ink)]">没有找到相关资料</p>
              <p className="text-sm text-[rgba(23,23,25,0.5)] font-body">换个关键词，或者让 AI 帮你找</p>
              <div className="flex gap-3 mt-2">
                <Button variant="secondary" onClick={() => onNavigate("discover", "default")}>
                  换个关键词
                </Button>
                <Button
                  variant="primary"
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-[clamp(24px,4vw,64px)] py-14 flex flex-col gap-14">
        ...
        <section className="flex flex-col gap-6">
          ...
          {chatTurns.length > 0 && (
            ...
            >
              <span>
                <strong className="block text-sm text-[var(--color-ink)]">继续上次对话</strong>
                <small className="mt-1 block text-[rgba(23,23,25,0.5)]">
                  已保留 {Math.ceil(chatTurns.length / 2)} 轮消息和当前用户画像
                </small>
              </span>
              <span aria-hidden="true" className="text-lg text-[var(--color-crimson)]">→</span>
            </button>
          )}
        ...
        <section className="flex flex-col gap-5">
          <h2 className="font-serif font-bold text-xl text-[var(--color-ink)]">最近更新</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {recentItems.map((item) => (
              <ResourceCard key={item.id} item={item} onOpen={onOpenDrawer} onSave={handleSave} />
            ))}
          </div>
        </section>
        ...
        {/* Trending */}
        <section className="flex flex-col gap-5">
          <h2 className="font-serif font-bold text-xl text-[var(--color-ink)]">大家正在看</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {trendItems.map((item) => (
              <ResourceCard key={item.id} item={item} onOpen={onOpenDrawer} onSave={handleSave} />
            ))}
