...
export function searchKnowledge(query: string, corpus: Corpus, limit = 5): SearchHit[] {
  return searchChunks(allChunks, query, corpus, limit).map(presentKnowledgeChunk);
}

function knowledgeHitsById(ids: string[]): SearchHit[] {
  return ids.flatMap((id, index) => {
    const chunk = allChunks.find((item) => item.id === id);
    return chunk ? [presentKnowledgeChunk({ ...chunk, score: 200 - index }) as SearchHit] : [];
  });
}

function mergeHits(...groups: SearchHit[][]): SearchHit[] {
  return groups.flat().filter(
    (hit, index, items) => items.findIndex((item) => item.id === hit.id) === index,
  );
}
...
function preferredResourcesForNetwork(): SearchHit[] {
  ...
  return preferences
    .map((title, index) => {
      ...
      return chunk ? presentKnowledgeChunk({ ...chunk, score: 100 - index }) : undefined;
    })
...
function preferredResourcesForFoundation(foundation?: string): SearchHit[] {
  const preferredIds: Record<string, string[]> = {
    ...
    '数学与统计': [
      'NJFU-RES-32D5277C00', 'NJFU-RES-3219992A28', 'NJFU-RES-78AA5C680C',
      'NJFU-RES-EB51A1F313', 'NJFU-RES-F2841FE4EC',
    ],
    '软件与产品开发': [
    ...
    '系统与底层开发': [
      'NJFU-RES-8FE4C06D13', 'NJFU-RES-C1EF3CEF13', 'NJFU-RES-940D1B3015',
      'NJFU-RES-C7EBEAC0CE', 'NJFU-RES-87481C00B1',
    ],
    '嵌入式与物联网': [
...
function lookupReply(input: string, profile: StudentProfile): AssistantReply {
  ...
  const asksForComposition = /推免.{0,8}(?:综合成绩)?.{0,8}(?:组成|构成)|(?:组成|构成).{0,8}推免/.test(input);
  const asksForHistoricalFivePoints = /(?:5\s*分|五分).{0,10}(?:以上|超过)|(?:以上|超过).{0,10}(?:5\s*分|五分)/.test(input);
  const asksAboutGpaThreshold = /GPA.{0,12}(?:40%|前\s*40)|(?:40%|前\s*40).{0,12}GPA/i.test(input);
  const policyHits = asksForComposition
    ...
    : asksForHistoricalFivePoints
      ...
      : asksAboutGpaThreshold
        ? knowledgeHitsById(['policy-md-002', 'policy-md-036', 'policy-md-003', 'policy-md-001', 'policy-md-004'])
        : searchKnowledge(input, 'policy', 5);
  const resourceHits = wantsResources ? searchKnowledge(input, 'resources', 6) : [];
...
export function answerUser(input: string, currentProfile: StudentProfile): AssistantReply {
  const asksForPersonalDecision = /(推荐|适合我|我适合|适合参加|应该(?:参加|选择|验证|先)|该参加|先看|先补|第一步|怎么开始|从哪里开始|哪个比赛开始|想参加.+比赛|是转.+还是|继续做.+方向)/.test(input);
  const asksForFacts = /(哪些|多少|几分|要求|规则|政策|资料|真题|课件|教材|模板|报告|由什么组成|怎么折算|有什么区别|能靠.+吗|是不是|是否|适合什么|适合作为|需要哪些能力)/.test(input);
  const isDirectLookup = asksForFacts && !asksForPersonalDecision;
  const profile = isDirectLookup ? currentProfile : updateProfile(currentProfile, input);

  if (isDirectLookup) return lookupReply(input, profile);

  const missing = missingRequiredFields(profile);
  const asksForContestDecision = asksForPersonalDecision && /(比赛|竞赛|大赛|挑战赛|蓝桥杯|建模|C4|ISCC|系统能力|转算法|网络方向)/i.test(input);
  const hasEnoughContestContext = Boolean(
    (profile.foundation || profile.interest)
    && asksForContestDecision
    && (profile.grade || profile.goal || profile.weeklyHours || profile.team
  ...
  if (missing.length > 0 && !hasEnoughContestContext) {
    return {
      ...
      text: `我已记下：${[
        ...
        profile.goal,
        profile.foundation,
        profile.interest ? `感兴趣：${profile.interest}` : undefined,
  ...
  if (direction === '计算机网络') {
    ...
    const stageHits = profile.grade === '大一'
    ...
    const confirmedContext = [
      profile.grade,
      profile.foundation ? `${profile.foundation}基础` : `${profile.interest}兴趣`,
      profile.weeklyHoursLabel,
      profile.team,
    ].filter(Boolean).join('、');
    return {
  ...
  const topicPolicyHits = /物联网/.test(input) && /机器人/.test(input)
    ? knowledgeHitsById(['policy-md-030', 'policy-md-031', 'policy-md-032', 'policy-md-033', 'policy-md-007'])
    : /(?:CTF|信息安全|CISC|ISCC)/i.test(input)
      ? knowledgeHitsById(['policy-md-026', 'policy-md-027', 'policy-md-028', 'policy-csv-06_竞赛政策对应速查-010', 'policy-csv-06_竞赛政策对应速查-014'])
      : /系统能力/.test(input)
        ? knowledgeHitsById(['policy-md-029', 'policy-md-007'])
        : [];
  const searchedPolicyHits = searchKnowledge(routeQuery, 'policy', 10).filter((hit) => {
  ...
  const searchedResources = searchKnowledge(`${input} ${direction} 课程 学习 练习 教材`, 'resources', 8);
